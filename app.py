...
def check_password():
    ...
    with col2:
        ...
        if st.button("Unlock System"):
            # This line now looks for a secret named 'INSTITUTIONAL_PASSWORD'
...
if st.button("🚀 Run Reconciliation"):
    if gl_uploads and csv_uploads:
        ...
        is_m = (nibss_review['Kachasi_ref'] != "") if 'Kachasi_ref' in nibss_review else pd.Series([False]*len(nibss_review))
        is_c = ((nibss_review['Kachasi_ref'] == "") & (nibss_review.get('mda_name', '') == 'NIGERIA CUSTOM SERVICES') & (nibss_review['unique_reference'].str.startswith('F', na=False)))
        ...
        with d1:
            ...
            st.table(df_bridge.style.format("₦{:,.2f}").applymap(lambda x: 'color: red; font-weight: bold' if x == bridging_diff and x != 0 else ''))
        with d2:
            ...
            st.table(df_comp.style.format("₦{:,.2f}").applymap(lambda x: 'color: red; font-weight: bold' if x == csv_vs_kachasi_diff and x != 0 else ''))
        with d3:
